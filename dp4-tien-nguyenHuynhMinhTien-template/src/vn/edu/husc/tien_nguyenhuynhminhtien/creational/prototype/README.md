...
- Phần 2 - Sao chép danh sách
    ...
    for (int i = 0; i < lst1.size(); i++) {
    		...
    		SinhVien x2;
    		x2 = new SinhVien(x); // Tạo bản sao x2 của x
    ...
    for (int i = 0; i < lst1.size(); i++) {
    	...
    	if (x instanceof SinhVienNuocNgoai)
    			x2 = new SinhVienVienNuocNgoai( (SinhVienNuocNgoai)x );
    	else
    			x2 = new (SinhVien(x);
...
Nói cách khác, để tạo bản sao cho đối tượng kiểu `SinhVien` hay lớp con của nó (như `SinhVienNuocNgoai`) chúng ta chỉ cần dùng code như sau:
...
4. Đối với những class kế thừa class này thì phải thực hiện hai việc
    ...
    class TenLopCon extends TenLop {
    		...
    		public TenLopCon(TenLopCon x) {
    				...
    				this.field3 = x.field3; // gán thêm th.tính mới, chỉ có ở lóp con
    		}
...
- Giống tư duy của mẫu **Factory Method** ở điểm: thay vì `if ...` để tạo mới `Product` thì tạo ra các lớp con
